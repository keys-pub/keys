...
func TestSearchUsers(t *testing.T) {
	...
	ids := []ID{}
	for i := 10; i < 15; i++ {
		key := NewEdX25519KeyFromSeed(Bytes32(bytes.Repeat([]byte{byte(i)}, 32)))
	...
	require.Equal(t, TimeMs(1234567890034), results[0].UserResult.VerifiedAt)
	require.Equal(t, TimeMs(1234567890033), results[0].UserResult.Timestamp)
	...
	// Add alice@twitter
	alice2 := NewEdX25519KeyFromSeed(Bytes32(bytes.Repeat([]byte{0x03}, 32)))
...
func TestSearchUsersRequestErrors(t *testing.T) {
	...
	require.Equal(t, TimeMs(1234567890003), results[0].UserResult.Timestamp)
	require.Equal(t, TimeMs(1234567890004), results[0].UserResult.VerifiedAt)
	...
	require.Equal(t, 1, len(results))
	require.NotNil(t, results[0].UserResult)
	require.Equal(t, UserStatusConnFailure, results[0].UserResult.Status)
	require.Equal(t, TimeMs(1234567890007), results[0].UserResult.Timestamp)
	require.Equal(t, TimeMs(1234567890004), results[0].UserResult.VerifiedAt)
...
func TestExpired(t *testing.T) {
	...
	require.Equal(t, "https://gist.github.com/alice/1", results[0].UserResult.User.URL)
	require.Equal(t, 1, results[0].UserResult.User.Seq)
	require.Equal(t, TimeMs(1234567890003), results[0].UserResult.VerifiedAt)
	require.Equal(t, TimeMs(1234567890002), results[0].UserResult.Timestamp)
...
func TestGenerateUserStatement(t *testing.T) {
	...
	dst := NewMem()
	scs := NewSigchainStore(dst)
	key := NewEdX25519KeyFromSeed(Bytes32(bytes.Repeat([]byte{0x01}, 32)))
