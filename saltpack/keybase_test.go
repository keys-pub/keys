...
func TestKeybaseMessageNoKey(t *testing.T) {
	...
	_, _, detected, err := saltpack.Open([]byte(msg), kr)
	require.Equal(t, saltpack.SigncryptEncoding, detected.Encoding)
	// TODO: Fix brand?
	require.Equal(t, "", detected.Brand)
	...
	_, _, detected, err = saltpack.Open([]byte(msg2), kr)
	require.Equal(t, saltpack.EncryptEncoding, detected.Encoding)
	// TODO: Fix brand?
	require.Equal(t, "", detected.Brand)
	require.EqualError(t, err, "no decryption key found for message")
}
